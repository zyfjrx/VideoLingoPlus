...
def align_subs(src_sub: str, tr_sub: str, src_part: str,GPT_LOG_FOLDER,console) -> Tuple[List[str], List[str], str]:
    ...
    parsed = ask_gpt(align_prompt, resp_type='json', valid_def=valid_align, log_title='align_subs',GPT_LOG_FOLDER=GPT_LOG_FOLDER)
...
def split_align_subs(src_lines: List[str], tr_lines: List[str],GPT_LOG_FOLDER,console):
    ...
    def process(i):
        split_src = split_sentence(src_lines[i], num_parts=2,GPT_LOG_FOLDER=GPT_LOG_FOLDER).strip()
        src_parts, tr_parts, tr_remerged = align_subs(src_lines[i], tr_lines[i], split_src,GPT_LOG_FOLDER,console)
...
def split_for_sub_main(video_name):
    console = get_console()
    ...
    for attempt in range(3):  # 多次切割
        ...
        split_src, split_trans, remerged = split_align_subs(src.copy(), trans,GPT_LOG_FOLDER,console)
    ...
    pd.DataFrame({'Source': split_src, 'Translation': split_trans}).to_excel(SPLIT_SUB, index=False)
    pd.DataFrame({'Source': src, 'Translation': remerged}).to_excel(REMERGED, index=False)

if __name__ == '__main__':
    split_for_sub_main("28679343296-1-16")
