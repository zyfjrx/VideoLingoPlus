...
def analyze_subtitle_timing_and_speed(df,RAW_AUDIO_FILE,console):
    console.print("[🔍 Analyzing] Calculating subtitle timing and speed...")
    # global ESTIMATOR
...
def process_cutoffs(df,console):
    console.print("[✂️ Processing] Generating cutoff points...")
    ...
    while idx < len(df):
        ...
        if df.iloc[idx]['cut_off'] == 1:
            if df.iloc[idx]['if_too_fast'] == 2:
                console.print(f"[⚠️ Warning] Line {idx} is too fast and cannot be fixed by speed adjustment")
            idx += 1
            continue
...
def gen_dub_chunks(video_name):
    ...
    console = get_console()
    console.print("[🎬 Starting] Generating dubbing chunks...")
    df = pd.read_excel(AUDIO_TASK)
    
    console.print("[📊 Processing] Analyzing timing and speed...")
    df = analyze_subtitle_timing_and_speed(df,RAW_AUDIO_FILE,console)
    
    console.print("[✂️ Processing] Processing cutoffs...")
    df = process_cutoffs(df,console)

    console.print("[📝 Reading] Loading transcript files...")
    ...
    for idx, row in df.iterrows():
        ...
        for i in range(last_idx, len(content_lines)):
            ...
            console.print(f"[❌ Error] Matching failed at line {idx}:")
    ...
    console.print("[✅ Complete] Matching completed successfully!")
