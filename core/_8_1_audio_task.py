...
def check_len_then_trim(text, duration,console):
    # global ESTIMATOR
    ...
    if estimated_duration > duration:
        console.print(Panel(f"Estimated reading duration {estimated_duration:.2f} seconds exceeds given duration {duration:.2f} seconds, shortening...", title="Processing", border_style="yellow"))
        original_text = text
        ...
        try:    
            ...
            console.print("[bold red]🚫 AI refused to answer due to sensitivity, so manually remove punctuation[/bold red]")
            shortened_text = re.sub(r'[,.!?;:，。！？；：]', ' ', text).strip()
        console.print(Panel(f"Subtitle before shortening: {original_text}\nSubtitle after shortening: {shortened_text}", title="Subtitle Shortening Result", border_style="green"))
...
def process_srt(TRANS_SUBS_FOR_AUDIO_FILE,SRC_SUBS_FOR_AUDIO_FILE,console):
    ...
    for block in content.strip().split('\n\n'):
        ...
        try:
            ...
            console.print(Panel(f"Unable to parse subtitle block '{block}', error: {str(e)}, skipping this subtitle block.", title="Error", border_style="red"))
            continue
    ...
    while i < len(df):
        ...
        if df.loc[i, 'duration'] < MIN_SUB_DUR:
            if i < len(df) - 1 and time_diff_seconds(df.loc[i, 'start_time'],df.loc[i+1, 'start_time'],today) < MIN_SUB_DUR:
                console.print(f"[bold yellow]Merging subtitles {i+1} and {i+2}[/bold yellow]")
                ...
                if i < len(df) - 1:  # Not the last audio
                    console.print(f"[bold blue]Extending subtitle {i+1} duration to {MIN_SUB_DUR} seconds[/bold blue]")
                    ...
                    console.print(f"[bold red]The last subtitle {i+1} duration is less than {MIN_SUB_DUR} seconds, but not extending[/bold red]")
                i += 1
        else:
            i += 1
...
# @check_file_exists(AUDIO_TASK)
def gen_audio_task_main(video_name):
    pathManager = PathManager(video_name)
    OUTPUT_DIR = _OUTPUT_DIR(pathManager)
    TRANS_SUBS_FOR_AUDIO_FILE = f"{OUTPUT_DIR}/audio/trans_subs_for_audio.srt"
    SRC_SUBS_FOR_AUDIO_FILE = f"{OUTPUT_DIR}/audio/src_subs_for_audio.srt"
    AUDIO_TASK = _8_1_AUDIO_TASK(pathManager)
    console = get_console()

    df = process_srt(TRANS_SUBS_FOR_AUDIO_FILE,SRC_SUBS_FOR_AUDIO_FILE,console)
    console.print(df)
    df.to_excel(AUDIO_TASK, index=False)
    console.print(Panel(f"Successfully generated {AUDIO_TASK}", title="Success", border_style="green"))

if __name__ == '__main__':
    gen_audio_task_main("808c5a77e3419eeb9017f6029e97a10a")