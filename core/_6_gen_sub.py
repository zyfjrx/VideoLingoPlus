...
def show_difference(str1, str2,console):
    ...
    console.print("Difference positions:")
    console.print(f"Expected sentence: {str1}")
    console.print(f"Actual match: {str2}")
    console.print("Position markers: " + "".join("^" if i in diff_positions else " " for i in range(max(len(str1), len(str2)))))
    console.print(f"Difference indices: {diff_positions}")

def get_sentence_timestamps(df_words, df_sentences,console):
    time_stamp_list = []
    ...
    for idx, sentence in df_sentences['Source'].items():
        ...
        if not match_found:
            ...
            show_difference(clean_sentence, 
                          full_words_str[current_pos:current_pos+len(clean_sentence)],console)
            console.print("\nOriginal sentence:", df_sentences['Source'][idx])
            raise ValueError("❎ No match found for sentence.")
    
    return time_stamp_list

def align_timestamp(df_text, df_translate, subtitle_output_configs: list, output_dir: str, for_display: bool = True,console=None):
    ...
    # Process timestamps ⏰
    time_stamp_list = get_sentence_timestamps(df_text, df_translate,console)
...
def align_timestamp_main(video_name):
    ...
    SPLIT_SUB = _5_SPLIT_SUB(pathManager)
    REMERGED = _5_REMERGED(pathManager)
    console = get_console()
    df_text = pd.read_excel(CLEANED_CHUNKS)
    df_text['text'] = df_text['text'].str.strip('"').str.strip()
    df_translate = pd.read_excel(SPLIT_SUB)
    ...
    # for audio
    df_translate_for_audio = pd.read_excel(REMERGED) # use remerged file to avoid unmatched lines when dubbing
...
if __name__ == '__main__':
    align_timestamp_main("28679343296-1-16")