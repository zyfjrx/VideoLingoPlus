...
def merge_subtitles_to_video(video_file):
    console = get_console()
    ...
    if not os.path.exists(SRC_SRT) or not os.path.exists(TRANS_SRT):
        console.print("Subtitle files not found in the 'output' directory.")
        exit(1)
    ...
    if ffmpeg_gpu:
        console.print("[bold green]will use GPU acceleration.[/bold green]")
        ffmpeg_cmd.extend(['-c:v', 'h264_nvenc'])
    ffmpeg_cmd.extend(['-y', OUTPUT_VIDEO])

    console.print("🎬 Start merging subtitles to video...")
...
if __name__ == "__main__":
    merge_subtitles_to_video("/home/bmh/VideoLingo/core/input/28679343296-1-16.mp4")